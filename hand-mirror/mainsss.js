...
const neckIndices = [152, 377, 400, 378, 379];
...
let neckPoints = createPartModel(0x996633, neckIndices.length);
...
neckPoints.renderOrder = 6;

scene.add(eyePoints, mouthPoints, nosePoints, browPoints, earPoints, neckPoints);
...
function updatePart(landmarks, indices, points) {
    ...
    for (let i = 0; i < indices.length; i++) {
        ...
        positions[i * 3 + 1] = mapY(lm.y) - 1.0; // 1 birlik pastga siljitish
...
faceMesh.onResults((results) => {
    ...
    updateShoulders(landmarks, shoulderPoints);
    ...
    updatePart(landmarks, neckIndices, neckPoints);
...
function createShoulderModel(color) {
    const geometry = new THREE.BufferGeometry();
    const positions = new Float32Array(2 * 3); // chap va o‘ng yelka
    geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    const material = new THREE.PointsMaterial({ color, size: 0.15 });
    const points = new THREE.Points(geometry, material);
    points.visible = true;
...
let shoulderPoints = createShoulderModel(0x996633); // yelka tan rang
scene.add(shoulderPoints);


function updateShoulders(landmarks, points) {
    const positions = points.geometry.attributes.position.array;

    // Chap yelka (jawline chap nuqtasi asosida)
    positions[0] = mapX(landmarks[234].x);
    positions[1] = mapY(landmarks[234].y) - 2.0; // pastga siljitish
    positions[2] = mapZ(landmarks[234].z);

    // O‘ng yelka (jawline o‘ng nuqtasi asosida)
    positions[3] = mapX(landmarks[454].x);
    positions[4] = mapY(landmarks[454].y) - 2.0;
    positions[5] = mapZ(landmarks[454].z);
